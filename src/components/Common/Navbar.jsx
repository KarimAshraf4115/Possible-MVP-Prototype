...
function Navbar({ onMenuClick }) {
  return (
    <header className="h-22.5 px-5 lg:px-7 flex items-center justify-between gap-3">
      {/* Hamburger — mobile only */}
      <button
        onClick={onMenuClick}
        className="lg:hidden w-11 h-11 shrink-0 rounded-full bg-white/70 border border-white/60 flex items-center justify-center"
      >
        <Menu size={20} strokeWidth={1.8} />
      </button>

      {/* Search */}
      <div className=" w-full lg:w-102.5 h-12 px-4 rounded-full bg-white/70 flex items-center gap-3 border border-white/60">
